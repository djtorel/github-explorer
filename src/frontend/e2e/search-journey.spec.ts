	...
	data: {
		items: [
			...
			},
			{
				...
				forksCount: 2,
				language: "Python",
				htmlUrl: "https://github.com/octocat/repo3",
			},
		],
		totalCount: 3,
	},
...
test.describe("Search Journey", () => {
	test("user can search for a GitHub profile and view repositories", async ({
		page,
	}) => {
		await page.goto("/#/");

		await expect(
			page.getByRole("heading", { name: /github explorer/i }),
		).toBeVisible();
